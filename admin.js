...
function openModal(id, userRole) {
  ...
  if (id === "modal-add-user" && userRole) {
    ...
    badge.style.cssText = isTeacher ? "background:rgba(64,96,240,0.15);color:#6888f8" : "background:var(--accent-muted);color:var(--accent)";
    const classLabel = document.getElementById("field-class-label");
    const classInput = document.getElementById("new-class");
    if (isTeacher) { classLabel.textContent = "Spécialité / Matière"; classInput.placeholder = "Ex: Électricité industrielle"; }
    else { classLabel.textContent = "Filière / Classe"; classInput.placeholder = "Ex: Électricité Industrielle — L2"; }
    ["new-fullname","new-email","new-password","new-class"].forEach(fid => {
...
async function createUser() {
  ...
  try {
    ...
    if (role === "teacher") { showAlert("teachers-alert", `✓ Compte professeur créé pour ${fullName}`, "success"); await loadTeachers(); }
    else { showAlert("students-alert", `✓ Compte élève créé pour ${fullName}`, "success"); await loadStudents(); }
  } catch (e) {
...
function renderStudents(list) {
  ...
  if (!tbody) return;
  if (!list.length) { tbody.innerHTML = `<tr><td colspan="5" class="table-loading">Aucun élève inscrit</td></tr>`; return; }
  tbody.innerHTML = list.map(s => `<tr>
    ...
    <td class="hide-mobile">
      <div class="progress-mini-wrap">
        ...
        <span style="font-size:0.78rem;color:var(--text-muted)">${s.progressPct || 0}%</span>
      </div>
    </td>
    <td>
      <div class="table-actions">
        <button class="btn-secondary btn-sm" onclick="viewStudent('${s.id}')">👁</button>
        <button class="btn-secondary btn-sm" onclick="openGradeModal('${s.id}','${s.fullName.replace(/'/g,"\\'")}')">📊</button>
        <button class="btn-secondary btn-sm" onclick="markAttendance('${s.id}','${s.fullName.replace(/'/g,"\\'")}')">📋</button>
        <button class="btn-secondary btn-sm" onclick="linkParent('${s.id}','${s.fullName.replace(/'/g,"\\'")}')">👨‍👩‍👧</button>
        <button class="btn-secondary btn-sm" style="color:var(--error)" onclick="confirmDelete('${s.id}','${s.fullName.replace(/'/g,"\\'")}','student')">🗑</button>
      </div>
    </td>
  </tr>`).join("");
}
...
function filterStudents() {
  ...
  renderStudents(allStudents.filter(s =>
    (s.fullName||"").toLowerCase().includes(q) || (s.email||"").toLowerCase().includes(q) || (s.class||"").toLowerCase().includes(q)
  ));
}
...
function renderTeachers(list) {
  ...
  if (!tbody) return;
  if (!list.length) { tbody.innerHTML = `<tr><td colspan="5" class="table-loading">Aucun professeur inscrit</td></tr>`; return; }
  tbody.innerHTML = list.map(t => {
    ...
    return `<tr>
      ...
      <td class="hide-mobile"><span class="badge">${courseCount}</span></td>
      <td><button class="btn-secondary btn-sm" style="color:var(--error)" onclick="confirmDelete('${t.id}','${t.fullName.replace(/'/g,"\\'")}','teacher')">🗑</button></td>
    </tr>`;
  }).join("");
}
...
function filterTeachers() {
  ...
  renderTeachers(allTeachers.filter(t =>
    (t.fullName||"").toLowerCase().includes(q) || (t.email||"").toLowerCase().includes(q) || (t.specialty||"").toLowerCase().includes(q)
  ));
}
...
function renderCoursesAdmin(list) {
  ...
  if (!grid) return;
  if (!list.length) { grid.innerHTML = `<div class="empty-state">📚 Aucun cours publié.</div>`; return; }
  grid.innerHTML = list.map(c => {
    ...
    return `<div class="course-card">
      ...
      <div style="font-size:0.75rem;color:var(--text-muted);margin-bottom:0.8rem">Par : ${author}</div>
...
async function createCourse() {
  ...
  if (!title || !content) { errEl.textContent = "Le titre et le contenu sont obligatoires."; errEl.classList.remove("hidden"); return; }
  errEl.classList.add("hidden");
  try {
    await window._addDoc(window._collection(window._db, "courses"), {
      title, description: desc, category: cat || "Général", content,
      authorId: window._currentUID, authorName: window._currentName, authorRole: window._currentRole,
      createdAt: window._serverTimestamp()
    });
    ...
    await loadCourses();
  } catch (e) { errEl.textContent = "Erreur lors de la publication."; errEl.classList.remove("hidden"); }
}

async function deleteCourse(id) {
  const course = allCourses.find(c => c.id === id);
  if (window._currentRole === "teacher" && course?.authorId !== window._currentUID) {
    showAlert("courses-alert", "Vous ne pouvez supprimer que vos propres cours.", "error"); return;
  }
...
async function loadQuizzes() {
  try {
    ...
    allQuizzesList = [];
    snap.forEach(d => { const q = d.data(); if (!q.deleted) allQuizzesList.push({ id: d.id, ...q }); });
...
function renderQuizzes(list) {
  ...
  if (!tbody) return;
  if (!list.length) { tbody.innerHTML = `<tr><td colspan="5" class="table-loading">Aucun quiz créé</td></tr>`; return; }
  tbody.innerHTML = list.map(q => {
    ...
    return `<tr>
      ...
      <td class="hide-mobile" style="color:var(--text-muted)">${courseTitle}</td>
      <td><span class="badge quiz-badge">${(q.questions||[]).length} Q</span></td>
      <td class="hide-mobile" style="font-size:0.82rem;color:var(--text-muted)">${q.authorName || "—"}</td>
      <td>${canDelete ? `<button class="btn-icon" onclick="deleteQuiz('${q.id}')">🗑</button>` : "—"}</td>
    </tr>`;
  }).join("");
}

function filterQuizzes() {
  const q = document.getElementById("search-quizzes").value.toLowerCase();
  renderQuizzes(allQuizzesList.filter(quiz =>
    (quiz.title||"").toLowerCase().includes(q) || (quiz.authorName||"").toLowerCase().includes(q)
  ));
}
...
async function createQuiz() {
  ...
  if (!questions.length) { errEl.textContent = "Ajoutez au moins une question complète."; errEl.classList.remove("hidden"); return; }
  try {
  ...
  try {
    ...
    document.getElementById("student-detail-content").innerHTML = `
      <div class="stats-grid" style="margin-bottom:1.2rem">
        ...
        <div class="stat-card"><div class="stat-icon">🏆</div><div class="stat-val">${avgScore !== null ? avgScore+"%" : "—"}</div><div class="stat-label">Score moy.</div></div>
      ...
      <p style="margin-bottom:0.4rem;font-size:0.88rem"><strong>Filière :</strong> ${data.class || "—"}</p>
      <p style="margin-bottom:1rem;font-size:0.88rem"><strong>Email :</strong> ${data.email}</p>
      <h4 style="font-family:'Syne',sans-serif;margin-bottom:0.8rem">Résultats quiz</h4>
      ${results.length === 0 ? '<p style="color:var(--text-muted);font-size:0.88rem">Aucun quiz complété.</p>' :
...
async function openGradeModal(studentId, studentName) {
  ...
  if (!subject) return;
  const valueStr = prompt(`Note /20 pour ${subject} :`);
  ...
  await window._setDoc(window._doc(window._db, "users", studentId, "grades", subject.toLowerCase().replace(/\s+/g,"-")), {
    subject, value, comment, updatedAt: window._serverTimestamp(),
    teacherId: window._currentUID, teacherName: window._currentName
  }, { merge: true });
  showAlert("students-alert", `✓ Note : ${studentName} — ${subject} : ${value != null ? value+"/20" : "—"}`, "success");
}

// ===== LIEN PARENT =====
async function linkParent(studentId, studentName) {
  const parentEmail = prompt(`Email du parent de ${studentName} :`);
  if (!parentEmail) return;
  const db = window._db;
  const snap = await window._getDocs(window._query(window._collection(db,"users"), window._where("email","==",parentEmail)));
  if (snap.empty) {
    const create = confirm(`Aucun compte trouvé pour ${parentEmail}.\nCréer un compte parent ?`);
    if (!create) return;
    const parentPass = prompt("Mot de passe (min 6 caractères) :");
